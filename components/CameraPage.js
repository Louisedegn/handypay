...
class CameraPage extends Component {
  constructor(props) {
    ...
    this.productRef = firebase.database().ref().child('products');
  }
  componentDidMount() {
    this.getLocation();
    AsyncStorage.getItem('user').then((userString) => {
      let user = JSON.parse(userString);
      this.setState({ uid: user.uid, disname: user.disname });
  ...
  _onBarCodeRead(e) {
    ...
    this.productRef.child(e.data).once('value', (snap) => {
      let product = {
        name: snap.val().name,
      };
      Alert.alert(e.data+' ; '+product.name);
      Actions.HomePage({ product: product });
    });
  }
  renderBarCodeInfo() {
    return (
